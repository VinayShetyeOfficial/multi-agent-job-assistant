...
function Index() {
  ...
  const gaps = useMemo(() => extractSection(agents.matcher.output, "Gaps"), [agents.matcher.output]);
        ...
        >
          <div className="space-y-5 lg:col-span-4">
            ...
            <p className="max-w-[38ch] text-sm leading-relaxed text-muted-foreground">
              Paste a job description. Watch a supervisor route a research,
              matcher, and prep agent — streaming reasoning in real time.
              Ends with a recruiter-ready case file.
            </p>
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:h-[720px]">
          ...
          <div className="flex flex-col rounded-xl border border-border bg-card p-5 lg:col-span-3">
            ...
            <div className="relative flex flex-1 flex-col items-center justify-center gap-8">
              ...
              <div className="relative w-full">
                ...
                <div className="absolute left-0 top-10 h-px w-full bg-border" />
                <div className="absolute left-1/6 top-10 h-12 w-px bg-border" />
                <div className="absolute left-1/2 top-10 h-12 w-px bg-border" />
                <div className="absolute left-5/6 top-10 h-12 w-px bg-border" />
              </div>
          ...
          <div className="scanline flex flex-col overflow-hidden rounded-xl border border-border bg-card lg:col-span-6">
            ...
            >
              {trace.length === 0 && !running && (
                <div className="flex h-full min-h-[300px] flex-col items-center justify-center gap-3 text-center text-muted-foreground">
                  ...
                  <p className="max-w-xs text-xs">
                    The trace will stream here once the supervisor dispatches
                    the first agent.
                  </p>
                </div>
              )}
              ...
              {AGENT_ORDER.filter((id) => agents[id].status === "running").map(
                (id) => (
                  <div
                    key={`stream-${id}`}
              ...
              {running &&
                AGENT_ORDER.every((id) => agents[id].status !== "running") && (
        ...
        >
          <div className="mb-10 text-center">
            ...
            <h2 className="mt-3 text-2xl font-bold md:text-3xl">
              Built on LangGraph cycles
            </h2>
            <p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground">
              A stateful supervisor routes tasks across specialized agents,
              each with their own tool set. State is shared, handoffs are
              explicit, and every step is streamed to the trace.
            </p>
          </div>
      ...
      {state.output ? (
        <div className="prose-agent break-words text-sm leading-7 text-foreground/90">
          <ReactMarkdown
            ...
            components={{
              h2: ({ node, ...props }) => (
                <h2 className="mt-6 mb-3 text-base font-semibold uppercase tracking-[0.2em] text-accent/90" {...props} />
              ),
              h3: ({ node, ...props }) => (
                <h3 className="mt-5 mb-2 text-sm font-semibold tracking-[0.08em] text-foreground" {...props} />
              ),
              p: ({ node, ...props }) => (
                <p className="mb-4 text-[0.95rem] leading-7 text-muted-foreground/90" {...props} />
              ),
              ul: ({ node, ...props }) => (
                <ul className="mb-4 ml-5 list-disc space-y-2 text-[0.95rem] leading-7 text-muted-foreground/90" {...props} />
              ),
              li: ({ node, ...props }) => (
